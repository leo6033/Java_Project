...
//        this.sendResponse(sendIO, response);
    ...
    private void sendFile(Response response) {
        ...
        try {
            ...
            bos.flush();
            synchronized (this) {
                ServerInfoFrame.appendTxt2MsgListArea("【文件消息】文件发送完毕!\n");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }finally{
    ...
    public void chat(Request request) throws IOException {
        ...
        ServerInfoFrame.appendTxt2MsgListArea(msg.getMessage());
    ...
    public static void toSendFile_(Request request)throws IOException{
        Response response = new Response();
        response.setStatus(ResponseStatus.OK);
        response.setType(ResponseType.TOSENDFILE);
        FileInfo sendFile = (FileInfo)request.getAttribute("file");
        ...
        //给文件接收方转发文件发送方的请求
        OnlineClientIOCache ioCache = DataBuffer.onlineUserIOCacheMap.get(sendFile.getToUser().getId());
        sendResponse_sys(ioCache, response);
    }
    ...
    /*向客户端发送图形信息*/
    public static void shape_info(Shape shape)throws IOException{

        Response response = new Response();
        response.setType(ResponseType.SHAPE);
        response.setData("Shape",shape);
